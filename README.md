...
- **永久下线**：一键永久下线服务器，保留历史财务和节点关系，同时停止余额消耗估算与到期提醒。
- **续费**：一键续费并延长到期时间，保留完整续费历史。
- **财务记录**：VPS 购买、续费和充值分别记录成本（USD）与实际付款（CNY）；客户收款使用独立台账，后台按客户汇总并计算差额（收款 − 实付）。
- **VPN 节点**：每台 VPS 下可记录多个节点（协议/地址/端口/配置），节点归属对应 VPS。
- **客户门户**（`/customer`）：管理员为每个客户分配独立账号；客户登录后只读查看名下 VPS、节点、付款/充值/收款和凭证，并可维护自己的终端用户及 VPN 节点分配。
- **到期推送**：运行中服务器剩余不足 5 天（可配）时自动推送 Telegram 提醒；客户收件人进入客户门户，全局收件人进入管理员客户详情。
- **管理后台**：单管理员密码登录，进行 VPS 增删改、永久下线、续费、节点、客户账号、终端用户分配与财务管理。
...
- 客户门户：`/customer`（管理员先在客户详情创建账号，客户首次登录必须修改初始密码）
...
| `CRON_SECRET` | 到期推送端点的调用凭证，建议 `openssl rand -hex 16`；未配置则推送端点禁用 |

> 说明：根目录 `.env` 同时被 Prisma CLI 与应用读取，生产环境把这些变量都放 `.env` 即可（最简单）。开发时也可用 `.env.local` 覆盖。

**到期推送**需再做两步：在 `/admin/settings` 填 Bot Token、站点地址（公网）并启用，在各客户详情页配置其 chat_id；然后加一条 crontab：

```bash
0 9 * * * curl -fsS -H "X-Cron-Secret: 你的CRON_SECRET" http://localhost:3000/api/cron/expiry-notify
```

详见 [docs/06-部署文档.md](docs/06-部署文档.md) 的「5c. 配置到期推送定时任务」。
