...
- **客户专属查看页**（无需登录，`/view/<客户ID>`）：客户可查看名下 VPS 的有效期（购买/到期时间、剩余天数彩色徽章）与可用 VPN 节点；**付款记录**（购买 + 续费 + 充值汇总，含凭证截图）与**收款记录**左右并排，方便逐笔对账。
- **到期推送**：服务器剩余不足 5 天（可配）时自动推送 Telegram 提醒，附客户专属查看链接。收件人**按客户单独配置**，另可配全局收件人（管理员）。
