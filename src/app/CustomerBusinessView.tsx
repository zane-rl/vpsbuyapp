...
export async function CustomerBusinessView({ customerId }: { customerId: string }) {
            ...
            {list.map((vps) => {
              ...
              return (
                <article key={vps.id} className="card card-hover animate-fade-in p-5">
                  ...
                  <Link href={`/customer/vps/${vps.id}`} className="mt-4 inline-flex text-sm font-medium text-indigo-600 hover:text-indigo-700 dark:text-indigo-400">
                    查看服务器详情与节点分配 →
                  </Link>
                </article>
              );
